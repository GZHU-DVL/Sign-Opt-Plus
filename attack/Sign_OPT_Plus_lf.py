    ...
    def attack_untargeted(self, x0, y0, eps, eta=0.2, alpha=1, beta=0.75, iterations=20000, query_limit=20000, h=0.001):
        ...
        for i in range(ini_iter):
            ...
            if model.predict_label(x0 + theta) != y0:
                theta_norm = torch.norm(theta, float('inf'))
                theta /= theta_norm
        ...
        for i in range(iterations):
            ...
            for _ in range(15):
                ...
                new_theta /= torch.norm(new_theta, float('inf'))
                ls_count += 1
            ...
            if (query_count > query_limit):
                ...
                dis = torch.norm(x_adv - x0, float('inf'))
                if(gg < eps):
                    suc_count += 1
